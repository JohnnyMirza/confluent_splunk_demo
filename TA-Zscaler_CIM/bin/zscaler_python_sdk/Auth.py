...
class Auth(object):
	...
	def get_settings_from_vars(self, username, password, apikey, debug = False):
		...
		self.zia_username = username
		self.zia_password = password
		self.zia_api_key = apikey
		return
	...
	def get_zia_creds_from_env(self, debug):			
		...
		if os.environ.get('ZIA_USERNAME') is not None:
			...
			logging.error("ENV IMPORT ERROR: {}".format("ZIA_USERNAME not found"))
			exit()
		...
		if os.environ.get('ZIA_PASSWORD') is not None:
			...
			logging.error("ENV IMPORT ERROR: {}".format("ZIA_PASSWORD not found"))
			exit()
		...
		if os.environ.get('ZIA_API') is not None:
			...
			logging.error("ENV IMPORT ERROR: {}".format("ZIA_API not found"))
			exit()
	...
	def set_cloud(self, cloud):
		...
		if cloud in Z_CLOUDS:
			...
			if self.debug:
				logging.debug("CLOUD ERROR: Unknown Cloud - {}".format(cloud))
